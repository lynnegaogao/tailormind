<template>
    <div class="container">
        <div ref="LearningPath" id="learningpath-area-after">
        </div>
        <div id="toolbar">
...
export default {
    ...
    methods: {
        drawLearningPath(learningPathData_After) {
            ...
            // 创建SVG元素
            const svg = d3.select('#learningpath-area-after').append('svg')
                .attr('width', width)
                .attr('height', height)
...
#toolbar {
    ...
    right: 80px;
    z-index: 10;
    /* Ensure this is higher than the z-index (if any) of learningpath-area-after */
    display: flex;
    gap: 8px;
}
